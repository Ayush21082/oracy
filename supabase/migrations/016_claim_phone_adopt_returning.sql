-- When a Firebase-verified phone already belongs to a profile that finished
-- onboarding, adopt that profile onto the current auth user so returning
-- phone logins skip the full onboarding flow (same idea as Apple identity reuse).
...
begin
  ...
  if prev.id is not null then
    -- Free the unique phone index before writing onto current.
    update public.profiles
       set phone = null
     where id = prev.id;

    if prev.onboarding_completed and not cur.onboarding_completed then
      -- Move practice data onto the live session (ignore date conflicts).
...
comment on function public.claim_verified_phone(text) is
  'Attaches a Firebase-verified phone to the current user; if that number belonged to a completed profile, adopts onboarding + practice data so returning users skip onboarding.';
